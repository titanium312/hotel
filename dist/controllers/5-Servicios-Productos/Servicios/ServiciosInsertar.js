...
exports.getEstadosCaja = exports.insertEstadoCaja = exports.recibirPedido = void 0;
const Database_1 = require("../../../db/Database");
const pool = Database_1.Database.connect();
const recibirPedido = async (req, res) => {
    const connection = await pool.getConnection();
    try {
        const { ID_Factura, Fecha_Emision, TipoFactura, ID_usuario, ID_MetodoPago, Descuento = 0, Adelanto = 0, Descripsion = '', servicios = [], } = req.body;
        ...
        for (const servicio of servicios) {
            ...
            const [productos] = await connection.query(`SELECT sp.ID_Producto, sp.Cantidad as CantidadPorServicio, p.Stock 
         FROM servicio_producto sp
         JOIN producto p ON sp.ID_Producto = p.ID_Producto
         WHERE sp.ID_Servicio = ?`, [servicio.ID_Servicio]);
        ...
        const serviciosAnteriores = new Map();
        if (ID_Factura) {
            const [detallesAntiguos] = await connection.query('SELECT ID_Servicio, Cantidad FROM servicio_detalle WHERE ID_Factura = ?', [ID_Factura]);
        ...
        if (ID_Factura) {
            await connection.query(`UPDATE factura SET Fecha_Emision=?, TipoFactura=?, ID_usuario=?, ID_MetodoPago=?, Descuento=?, Adelanto=?, Descripsion=?
         WHERE ID_Factura=?`, [Fecha_Emision, TipoFactura, ID_usuario, ID_MetodoPago, Descuento, Adelanto, Descripsion, ID_Factura]);
        }
        else {
            const [result] = await connection.query(`INSERT INTO factura (Fecha_Emision, TipoFactura, ID_usuario, ID_MetodoPago, Descuento, Adelanto, Descripsion, ID_estadoFactura)
         VALUES (?, ?, ?, ?, ?, ?, ?, 1)`, [Fecha_Emision, TipoFactura, ID_usuario, ID_MetodoPago, Descuento, Adelanto, Descripsion]);
        ...
        for (const [idServicioAntiguo, cantidadAntigua] of serviciosAnteriores) {
            if (!serviciosNuevosIds.has(idServicioAntiguo)) {
                ...
                const [productos] = await connection.query(`SELECT ID_Producto, Cantidad FROM servicio_producto WHERE ID_Servicio=?`, [idServicioAntiguo]);
                for (const producto of productos) {
                    const cantidadAjuste = cantidadAntigua * producto.Cantidad;
                    await connection.query(`UPDATE producto SET Stock = Stock + ? WHERE ID_Producto = ?`, [cantidadAjuste, producto.ID_Producto]);
                }
            }
        }
        ...
        for (const servicio of servicios) {
            ...
            const [productos] = await connection.query(`SELECT ID_Producto, Cantidad FROM servicio_producto WHERE ID_Servicio=?`, [servicio.ID_Servicio]);
            for (const producto of productos) {
            ...
            const [datosServicio] = await connection.query(`SELECT Precio FROM servicio WHERE ID_Servicio=?`, [servicio.ID_Servicio]);
            ...
            await connection.query(`INSERT INTO servicio_detalle (ID_Factura, ID_Servicio, Cantidad, Total, mesa)
         VALUES (?, ?, ?, ?, ?)`, [nuevaFacturaID, servicio.ID_Servicio, cantidadNueva, total, servicio.mesa || null]);
        }
...
const insertEstadoCaja = async (req, res) => {
    try {
        ...
        const [rows] = await conn.execute(`SELECT ID_estado, Comentario FROM estado_caja WHERE DATE(Fecha) = ?`, [fechaStr]);
        ...
        if (rows.length > 0) {
            ...
            await conn.execute(`UPDATE estado_caja SET Descripcion = ?, Comentario = ? WHERE ID_estado = ?`, [descripcion, comentarioActualizado, ID_estado]);
            conn.release();
            ...
            await conn.execute(`INSERT INTO estado_caja (Descripcion, Fecha, Comentario) VALUES (?, ?, ?)`, [descripcion, fechaParaInsertar, comentarioInsert]);
            conn.release();
...
exports.insertEstadoCaja = insertEstadoCaja;
const getEstadosCaja = async (req, res) => {
    try {
        const conn = await pool.getConnection();
...
exports.getEstadosCaja = getEstadosCaja;
