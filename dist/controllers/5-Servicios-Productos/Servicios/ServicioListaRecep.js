...
const ServicioListaRecep = async (req, res) => {
    try {
        ...
        const idFacturaRaw = req.query.idFactura;
    ...
    `;
        ...
        if (fechaInicio && fechaInicio.toLowerCase() !== 't' &&
        ...
        const servicioIds = Array.from(new Set(rows.map((r) => r.ID_Servicio)));
        let preciosMap = {};
        if (servicioIds.length > 0) {
            const [preciosRows] = await connection.query(`SELECT ID_Servicio, Precio FROM servicio WHERE ID_Servicio IN (${servicioIds.join(',')})`);
            preciosMap = preciosRows.reduce((acc, cur) => {
        ...
        const calcularPrecioUnitario = (item) => {
            ...
            if (precioTabla && item.Cantidad > 0)
                return precioTabla;
            if (item.Cantidad > 0)
                return Number(item.Total) / item.Cantidad;
            return 0;
        };
