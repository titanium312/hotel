...
exports.eliminarServicioDeFactura = exports.eliminarFacturaYDetalles = exports.actualizarEstadoFactura = void 0;
...
const eliminarFacturaYDetalles = async (req, res) => {
    ...
    try {
        ...
        await connection.execute('DELETE FROM servicio_detalle WHERE ID_Factura = ?', [idFactura]);
        // Elimina la factura
        const [result] = await connection.execute('DELETE FROM factura WHERE ID_Factura = ?', [idFactura]);
        // 'result' es un array con un objeto ResultSetHeader, donde podemos encontrar affectedRows
        const affectedRows = result.affectedRows;
