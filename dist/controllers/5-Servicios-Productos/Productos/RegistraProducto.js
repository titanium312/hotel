...
const createProduct = async (req, res) => {
    ...
    const { ID_Producto, nombre, descripcion, Precio_Unitario, Stock = 0, // Valor por defecto si no se envía
    ID_Provedor, ID_producto_tipo } = req.body;
    ...
    try {
        ...
        const [existingProduct] = await connection.execute('SELECT ID_Producto FROM producto WHERE ID_Producto = ?', [ID_Producto]);
        ...
        await connection.execute('INSERT INTO producto (ID_Producto, Nombre, Descripcion, Precio_Unitario, Stock, ID_Provedor, ID_producto_tipo) VALUES (?, ?, ?, ?, ?, ?, ?)', [ID_Producto, nombre, descripcion, Precio_Unitario, Stock, ID_Provedor, ID_producto_tipo]);
