...
const ProductoSale = async (req, res) => {
    ...
    try {
        ...
        // 1. Crear la factura
        const [result] = await connection.query(`INSERT INTO factura (Fecha_Emision, ID_estadoFactura, TipoFactura, Descuento, Adelanto)
       VALUES (?, 1, ?, ?, ?)`, [fechaEmision, tipoFactura, descuento, adelanto]);
        ...
        for (const item of productos) {
         ...
         VALUES (?, ?, ?, ?)`, [idFactura, id_producto, cantidad, fechaEmision]);
            // Actualizar el stock del producto
            await connection.query(`UPDATE producto SET Stock = Stock - ? WHERE ID_Producto = ?`, [cantidad, id_producto]);
        }
        // 3. Aplicar el descuento (si corresponde)
        totalVenta = totalVenta * (1 - descuento / 100);
        // 4. Actualizar el total de la factura
        await connection.query(`UPDATE factura SET Total = ? WHERE ID_Factura = ?`, [totalVenta, idFactura]);
        // 5. Si hay adelanto, restarlo del total
        if (adelanto > 0) {
            await connection.query(`UPDATE factura SET Total = Total - ? WHERE ID_Factura = ?`, [adelanto, idFactura]);
        }
...
exports.ProductoSale = ProductoSale;
