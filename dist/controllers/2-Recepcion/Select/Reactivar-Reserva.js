...
exports.ReactivarReserva = void 0;
...
class ReactivarReserva {
    ...
    static async reactivarReserva(req, res) {
        ...
        try {
            ...
            const [reserva] = await pool.query(`SELECT r.ID_Reserva, r.Fecha_Ingreso, r.Fecha_Salida, re.Descripcion AS EstadoDescripcion
         FROM Reserva r
         ...
         WHERE r.ID_Reserva = ?`, [idReserva]);
            ...
            const [conflictoReserva] = await pool.query(`SELECT * FROM Reserva
         WHERE Fecha_Entrada = ? AND Estado != 4 AND ID_Reserva != ?`, [reservaData.Fecha_Entrada, idReserva] // Evitar la misma reserva
            );
            if (conflictoReserva.length > 0) {
            ...
            if (horasDiferencia <= 5) {
                const [habitaciones] = await pool.query(`SELECT h.ID_Habitacion, h.Nombre, h.ID_Estado_Habitacion
           FROM habitacion h
         ...
         WHERE ID_Reserva = ? AND Estado = 4`, // Cambiar estado a 'Activo'
            ...
            if (horasDiferencia < 5) {
                const [updateHabitacionesResult] = await pool.query(`UPDATE habitacion h
           JOIN ReservaHabitacion rh ON rh.ID_Habitacion = h.ID_Habitacion
           ...
           WHERE rh.ID_Reserva = ?;`, [idReserva]);
                const updateHabitaciones = updateHabitacionesResult;
            ...
            const [updateFacturaResult] = await pool.query(`UPDATE factura f
         JOIN reserva r ON f.ID_Factura = r.ID_Factura
         SET f.ID_estadoFactura = 1  -- Estado 'Pendiente' (ID 1)
         WHERE r.ID_Reserva = ?;`, [idReserva]);
            const updateFactura = updateFacturaResult;
...
exports.ReactivarReserva = ReactivarReserva;
